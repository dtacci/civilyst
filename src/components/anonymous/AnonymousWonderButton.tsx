...
export function AnonymousWonderButton({ 
  ...
  const [category, setCategory] = useState<WonderCategory>(WonderCategory.GENERAL);
  const [timeContext, setTimeContext] = useState<WonderTimeContext>(WonderTimeContext.ANYTIME);
  const [shareLocation, setShareLocation] = useState(false);
  const [location, setLocation] = useState<{ lat: number; lng: number } | null>(null);

  const createWonderMutation = api.wonders.createAnonymous.useMutation({
    onSuccess: () => {
      toast.success('Wonder shared! 🌟 Your anonymous wonder has been shared with the community.');
      setIsOpen(false);
      resetForm();
    },
    onError: (error) => {
  ...
  const handleLocationToggle = async () => {
    if (!shareLocation && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          toast.error('Location access denied. You can still share your wonder without location.');
        }
      );
    } else {
      setShareLocation(false);
  ...
  const handleSubmit = async () => {
    if (!content && !voiceUrl) {
      toast.error('Please share your wonder. Use text or voice to describe what you\'re wondering about.');
      return;
    }

    if (!isFingerprintingAvailable()) {
      toast.error('Browser not supported. Please use a modern browser to share anonymous wonders.');
      return;
    }
      ...
      {isOpen && (
        ...
        >
          ...
          >
              ...
              {/* Time Context */}
              <div className="mb-4">
                <label className="text-sm font-medium mb-2 block">When is this relevant?</label>
                <div className="grid grid-cols-3 gap-2">
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {shareLocation ? 'Location shared' : 'Share location (optional)'}
                </Button>
                {shareLocation && (
              ...
              <Button
                ...
                disabled={createWonderMutation.isPending || (!content && !voiceUrl)}
                className="w-full"
                size="lg"
              >
