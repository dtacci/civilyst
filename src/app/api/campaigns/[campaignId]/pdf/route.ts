...
const PDFGenerationRequestSchema = z.object({
  type: z.enum(['full_report', 'voting_summary', 'qr_share']).default('full_report'),
  data: z.object({
    id: z.string(),
    title: z.string(),
...
const PDFQuerySchema = z.object({
  type: z.enum(['full_report', 'voting_summary', 'qr_share']).optional().default('full_report'),
  ...
  try {
    ...
    switch (type) {
      case 'full_report':
        ...
        pdfBuffer = await generateCampaignReport(reportData, options as any);
  ...
  try {
    ...
    if (download === 'true') {
      ...
      const minimalData: CampaignReportData = {
        ...
        description: 'This is a simplified report. Use the POST endpoint with full campaign data for complete reports.',
        status: 'ACTIVE',
        createdAt: new Date(),
        updatedAt: new Date(),
      };
      ...
      const downloadFilename = filename || `campaign-${campaignId}-basic-report.pdf`;
