...
class PWAEnhancedManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('campaigns')) {
          ...
          campaignStore.createIndex('timestamp', 'timestamp', { unique: false });
          campaignStore.createIndex('lastAccessed', 'lastAccessed', { unique: false });
        }
        ...
        if (!db.objectStoreNames.contains('offlineDrafts')) {
          const draftStore = db.createObjectStore('offlineDrafts', { keyPath: 'id' });
   ...
   */
  async addToSyncQueue(type: OfflineQueueItem['type'], data: unknown): Promise<string> {
   ...
   * Get offline drafts
   */
  async getOfflineDrafts(): Promise<Array<{ id: string; data: unknown; timestamp: number }>> {
    if (!this.db) return [];
