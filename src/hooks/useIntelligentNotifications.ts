...
export function useIntelligentNotifications(
  options: UseIntelligentNotificationsOptions = {}
) {
   ...
   */
  const notifyCampaignCreated = useCallback(
    async (campaign: {
      id: string;
      title: string;
      description: string;
   ...
   */
  const notifyVoteMilestone = useCallback(
    async (campaign: {
      id: string;
      title: string;
      voteCount: number;
   ...
   */
  const notifyEngagementStreak = useCallback(
    async (user: {
      id: string;
      streakDays: number;
   ...
   */
  const notifyEmergencyAlert = useCallback(
    async (alert: {
      title: string;
      message: string;
      location?: {
  ...
  const sendTestNotification = useCallback(
    async (type: NotificationType = 'campaign_created') => {
      try {
        const testPayloads: Record<
          NotificationType,
          Partial<IntelligentNotificationPayload>
        > = {
          campaign_created: {
